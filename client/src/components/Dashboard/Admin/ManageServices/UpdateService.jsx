...
const UpdateService = () => {
  ...
  const [serviceImg, setServiceImg] = useState(
    service?.serviceImg?.url?.length > 0 ? service?.serviceImg?.url : ''
  );
  const [serviceImgPreview, setServiceImgPreview] = useState(
    service?.serviceImg?.url?.length > 0 ? service?.serviceImg?.url : ''
  );
  const [selectImg, setSelectImg] = useState(
    service?.serviceImg?.url?.length > 0 ? true : false
  );
  ...
  return (
    <>
      <MetaDataTitle title={`Update ${service?.title} - Dr. Abdul Kader`} />
        ...
        <form className='p-1 md:p-4 w-full' onSubmit={handleSubmit(onSubmit)}>
          ...
          <input
            ...
            {...register('serviceID', {
              ...
              required: {
                value: true,
                message: 'Please fill up the Service ID field',
              },
              minLength: {
                value: 1,
